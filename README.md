...
Input: A set of N cities, and a distance matrix D where D<sub>ij</sub> is the distance (or cost) of traveling from city i to city j.
...
The core idea from the paper is to represent the distances D<sub>ij</sub> as phases. Create a matrix B where:
...
The construction of the unitary U is one of the most critical and potentially challenging aspects. The paper's approach involves creating a set of smaller unitary operators, U<sub>j</sub>, for each city.

[Uj]kk = (1 / sqrt(N)) * B[j][k] (where j, k range from 1 to N)

The rest of the elements in U<sub>j</sub> are initialized to zero. This creates a diagonal matrix, which is important.
...
The size of U will be 2<sup>number of qubits needed</sup>.
...
Control Unitary: Apply the controlled-U operator multiple times (controlled-U<sup>2^j</sup> for different values of j) on the eigenstate, where the control qubits are the qubits used for phase estimation.
...
Representing cities (log<sub>2</sub>(N) qubits).
...
Implementing the controlled unitary operators (U, U<sup>2</sup>, U<sup>4</sup>, ...).
...
Unitary Implementation: Constructing and implementing the controlled unitary operators (U, U<sup>2</sup>, U<sup>4</sup>, ...) efficiently is a significant challenge. The tensor product structure can lead to very complex circuits. The use of advanced circuit synthesis techniques may be necessary.
